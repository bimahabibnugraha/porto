  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-white p-6 md:p-16 pb-32 font-sans overflow-x-hidden selection:bg-purple-500 selection:text-white">
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-fit px-4">
        <div className="flex items-center justify-center gap-2 px-6 py-3 bg-neutral-900/80 backdrop-blur-xl border border-white/10 rounded-full shadow-2xl">
          {socialLinks.map((link, i) => (
            <Link 
              key={i} 
              href={link.href} 
              target={link.label === "Home" ? "_self" : "_blank"}
              className="p-2 rounded-full hover:bg-white/10 transition-all text-neutral-400 hover:text-white hover:scale-110"
            >
